...
public class OverlappingIntervalPartitionUtil {
    ...
    public static long getPartitionStartValue(RangeMap rangeMap, int pid, int nPartitions) {
        ...
        if (slotId < 0) {
            return LongPointable.getLong(rangeMap.getMinByteArray(), rangeMap.getMinStartOffset(0) + 1);
        } else {
            return LongPointable.getLong(rangeMap.getByteArray(), rangeMap.getStartOffset(0, slotId) + 1);
        }
    }

    public static long getPartitionEndValue(RangeMap rangeMap, int pid, int nPartitions) {
        int slotId = rangeMap.getMaxSlotFromPartition(pid, nPartitions);
        if (slotId >= rangeMap.getSplitCount()) {
            // Last available slot.
            return LongPointable.getLong(rangeMap.getMaxByteArray(), rangeMap.getMaxStartOffset(0) + 1);
        } else {
            return LongPointable.getLong(rangeMap.getByteArray(), rangeMap.getStartOffset(0, slotId) + 1);
        }
    }
